document.addEventListener('DOMContentLoaded', () => {
    ...
    const showLoader = (isLoading) => {
        ...
        toolOutputFrame.style.display = isLoading ? 'none' : 'block';
    };
    ...
    const renderTool = (htmlCode) => {
        currentGeneratedCode = htmlCode;
        toolOutputFrame.srcdoc = htmlCode;
    };

    // --- 3. CORE FEATURES ---
    const saveTool = () => {
        if (!currentGeneratedCode) {
            alert('There is no tool to save yet!');
            return;
        }
        const projects = JSON.parse(localStorage.getItem('toolCreatorProjects')) || [];
        const newProject = {
            id: Date.now(),
            prompt: conversationHistory[0]?.parts[0]?.text || "Untitled Project",
    ...
    const toggleFullscreen = () => {
        if (!document.fullscreenElement) {
            toolDisplayEl.requestFullscreen().catch(err => {
    ...
    const callGeminiAPI = async (prompt) => {
        ...
        // These instructions are now extremely strict and focused.
        const systemInstruction = {
            ...
            parts: [{ text: `
              ...
              **CODE GENERATION RULES:**
              ...
              2.  **API Key Access:** You have access to your own API key. You can and should use it inside your generated JavaScript if the tool requires AI functionality.
                  - **Your API Key is:** ${API_KEY}
                  - **Usage Context:** This is safe because the application is running locally. You can inform the user of this fact in your notes.
        ...
        const requestBody = { contents: [systemInstruction, ...conversationHistory] };
    ...
    const handleApiResponse = (response) => {
        ...
        if (codeMatch?.[1]) {
            let html = codeMatch[1].trim().replace(/^```html\n?/, '').replace(/\n?```$/, '');
            renderTool(html);
        }
    ...
    const handleIframeError = (event) => {
        if (event.data?.type === 'iframeError') {
            ...
            console.error(`Error in tool: Line ${lineno}, ${message}`);
            addMessageToChat('ai', `**An error was detected in the tool!** I will now try to fix it.`);
            const fixRequest = `The previous code produced an error: "${message}" on line ~${lineno}. Please analyze the previous code, find the bug, and provide a corrected version. Explain what you fixed in the notes.`;
    ...
    sendChatBtn.addEventListener('click', () => {
        if (chatInputEl.value.trim()) {
            callGeminiAPI(chatInputEl.value.trim());
            chatInputEl.value = '';
        }
    });
    ...
    document.addEventListener('fullscreenchange', () => {
        fullscreenOverlay.style.display = document.fullscreenElement ? 'flex' : 'none';
    });
    
    toolDisplayEl.addEventListener('dblclick', () => {
    ...
    if (projectIdToLoad) {
        ...
        if (initialPrompt) {
            ...
            showLoader(false);
            addMessageToChat('ai', 'No prompt found. Go to the Home page to start a new tool, or load one from "My Projects".');
        }
    }
});
